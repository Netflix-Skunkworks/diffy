...
def baseline(target_key: str,
    ...
    for k, v in results.items():
        persistence_plugin['options'].update(kwargs)
        persistence_plugin['plugin'].save('baseline', target_key, v[0], **persistence_plugin['options'])  # only one baseline
