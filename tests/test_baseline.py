...
def test_baseline_list_put(client, token, status):
    assert client.put(api.url_for(BaselineList), data={}, headers=token).status_code == status
...
def test_baseline_list_patch(client, token, status):
    assert client.patch(api.url_for(BaselineList), data={}, headers=token).status_code == status
...
def test_baseline_post(client, token, status):
    assert client.post(api.url_for(Baseline, key='foo'), data={}, headers=token).status_code == status
...
def test_baseline_put(client, token, status):
    assert client.put(api.url_for(Baseline, key='foo'), data={}, headers=token).status_code == status
...
def test_baseline_delete(client, token, status):
    assert client.delete(api.url_for(Baseline, key='foo'), headers=token).status_code == status
...
def test_baseline_patch(client, token, status):
    assert client.patch(api.url_for(Baseline, key='foo'), data={}, headers=token).status_code == status
