...
def test_analysis_list_put(client, token, status):
    assert client.put(api.url_for(AnalysisList), data={}, headers=token).status_code == status
...
def test_analysis_list_patch(client, token, status):
    assert client.patch(api.url_for(AnalysisList), data={}, headers=token).status_code == status
...
def test_analysis_post(client, token, status):
    assert client.post(api.url_for(Analysis, key='foo'), data={}, headers=token).status_code == status
...
def test_analysis_put(client, token, status):
    assert client.put(api.url_for(Analysis, key='foo'), data={}, headers=token).status_code == status
...
def test_analysis_delete(client, token, status):
    assert client.delete(api.url_for(Analysis, key='foo'), headers=token).status_code == status
...
def test_analysis_patch(client, token, status):
    assert client.patch(api.url_for(Analysis, key='foo'), data={}, headers=token).status_code == status
